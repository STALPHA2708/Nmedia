...
async function getMockResponse<T>(endpoint: string, options: RequestInit): Promise<ApiResponse<T>> {
  // Simulate API delay
  ...
  switch (endpoint) {
    case '/employees':
      if (method === 'GET') return getMockEmployees() as Promise<ApiResponse<T>>;
      if (method === 'POST') {
    ...
    case '/invoices':
      ...
      if (method === 'POST') {
        ...
        const amount = newInvoice.items.reduce((sum: number, item: any) => sum + item.total, 0);
        ...
        const invoice = {
          ...
          estimatedCosts: newInvoice.profitMargin ? amount * (1 - newInvoice.profitMargin / 100) : amount * 0.7,
          assignedEmployees: [],
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        };
        mockInvoices.unshift(invoice);
        return { success: true, data: invoice, message: 'Facture créée avec succès' } as ApiResponse<T>;
      }
      break;
    ...
    default:
      ...
      if (method === 'PUT') {
        if (endpoint.startsWith('/projects/')) {
          ...
          if (index > -1) {
            ...
            mockProjects[index] = {
              ...
              project_type: updateData.projectType || mockProjects[index].project_type,
              deliverables: updateData.deliverables || mockProjects[index].deliverables,
              notes: updateData.notes || mockProjects[index].notes,
              client_contact_name: updateData.clientContact?.name || mockProjects[index].client_contact_name,
              client_contact_email: updateData.clientContact?.email || mockProjects[index].client_contact_email,
              client_contact_phone: updateData.clientContact?.phone || mockProjects[index].client_contact_phone,
              progress: updateData.progress || mockProjects[index].progress,
              spent: updateData.spent || mockProjects[index].spent,
              updated_at: new Date().toISOString(),
            };
            return { success: true, data: mockProjects[index], message: 'Projet mis à jour avec succès' } as ApiResponse<T>;
          }
        }
        if (endpoint.startsWith('/employees/')) {
          ...
          if (index > -1) {
            ...
            return { success: true, data: mockEmployees[index], message: 'Employé mis à jour avec succès' } as ApiResponse<T>;
          }
        }
        if (endpoint.startsWith('/invoices/')) {
          ...
          if (index > -1) {
            ...
            return { success: true, data: mockInvoices[index], message: 'Facture mise à jour avec succès' } as ApiResponse<T>;
          }
        }
      }
      ...
      if (method === 'DELETE') {
        if (endpoint.startsWith('/employees/')) {
          ...
          return { success: true, message: 'Employé supprimé avec succès' } as ApiResponse<T>;
        }
        if (endpoint.startsWith('/projects/')) {
          const id = parseInt(endpoint.split('/')[2]);
          const index = mockProjects.findIndex(p => p.id === id);
          ...
          return { success: true, message: 'Projet supprimé avec succès' } as ApiResponse<T>;
        }
        if (endpoint.startsWith('/invoices/')) {
          const id = parseInt(endpoint.split('/')[2]);
          const index = mockInvoices.findIndex(i => i.id === id);
          ...
          return { success: true, message: 'Facture supprimée avec succès' } as ApiResponse<T>;
        }
      }
      break;
  }
...
export const employeeApi = {
  ...
  getStats: () => apiRequest<{ general: EmployeeStats; contractTypes: any[] }>('/employees/stats'),
};
...
export const projectApi = {
  ...
  getStats: () => apiRequest<{ general: ProjectStats; priorities: any[] }>('/projects/stats'),

  // Assign employee to project
  assignEmployee: (projectId: number, employeeId: number, data: {
    role: string;
    startDate: string;
    endDate?: string;
    hourlyRate?: number;
...
export const departmentApi = {
  ...
  // Update department
  update: (id: number, departmentData: { name: string; description?: string }) =>
...
export const contractTypeApi = {
  ...
  create: (data: Omit<ContractType, 'id' | 'created_at' | 'updated_at' | 'employee_count'>) =>
  ...
  update: (id: number, data: Partial<Omit<ContractType, 'id' | 'created_at' | 'updated_at' | 'employee_count'>>) =>
  ...
  delete: (id: number) => apiRequest<void>(`/contract-types/${id}`, {
    method: 'DELETE',
  }),
};
